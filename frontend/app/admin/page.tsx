...
export default function AdminDashboard() {
  ...
  const uCustomer = overview?.users.customer ?? 0;
  const uUmkm     = overview?.users.umkm ?? 0;
  const uAdmin    = overview?.users.admin_bumdes ?? 0;
  const uPengirim = uTotal > 0 ? Math.max(0, uTotal - uCustomer - uUmkm - uAdmin - (overview?.users.super_admin ?? 0)) : 0;
  const donutSeries = uTotal > 0 ? [uCustomer, uUmkm, uAdmin, uPengirim] : [62, 24, 8, 6];

  const pct = (n: number) => uTotal > 0 ? `${Math.round((n / uTotal) * 100)}%` : "—";
  const donutLegend = [
    { label: "Customer",      pct: pct(uCustomer), color: "bg-indigo-500" },
    { label: "UMKM / Seller", pct: pct(uUmkm),     color: "bg-green-600"  },
    { label: "Admin BUMDes",  pct: pct(uAdmin),    color: "bg-yellow-400" },
    { label: "Pengirim",      pct: pct(uPengirim), color: "bg-orange-500" },
  ];
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Pantau seluruh aktivitas platform BumDesMartNukita</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        ...
        <ApexChart key={activeChart} type="area" series={[{ name: chart.label, data: chart.data }]} options={areaOptions} height={220} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <ApexChart type="donut" series={donutSeries} options={{
            chart: { type: "donut", animations: { speed: 400 } },
            colors: ["#6366f1", "#16a34a", "#f59e0b", "#f97316"],
            labels: ["Customer", "UMKM / Seller", "Admin BUMDes", "Pengirim"],
            legend: { show: false },
            dataLabels: { enabled: false },
            plotOptions: { pie: { donut: { size: "65%", labels: { show: true, total: { show: true, label: "Pengguna", fontSize: "11px", color: "#6b7280", formatter: () => uTotal > 0 ? uTotal.toLocaleString("id-ID") : "—" } } } } },
      ...
      <div className="bg-white rounded-2xl border border-gray-100">
        ...
        <div className="overflow-x-auto">
          {users.length === 0 ? (
            <div className="px-5 py-8 text-center text-xs text-gray-400">Memuat data pengguna...</div>
          ) : (
