...
export default function ChatPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      {/* Banner "Segera Hadir" */}
      <div className="mb-4 flex items-start gap-3 px-4 py-3.5 rounded-xl border" style={{ background: "#FFF7ED", borderColor: "#FED7AA" }}>
        <span className="text-lg shrink-0">🚧</span>
        <div>
          <p className="text-sm font-semibold" style={{ color: "#92400E" }}>Fitur Chat Segera Hadir</p>
          <p className="text-xs mt-0.5" style={{ color: "#B45309" }}>
            Fitur pesan langsung dengan seller sedang dalam pengembangan. Tampilan di bawah adalah preview desain antarmuka yang akan tersedia segera.
            Sementara itu, hubungi seller melalui WhatsApp yang tersedia di halaman toko.
          </p>
        </div>
      </div>
      ...
      <div className="relative bg-white rounded-2xl border border-gray-100 overflow-hidden flex" style={{ height: "calc(100vh - 280px)", minHeight: 500 }}>
        {/* Overlay disabled */}
        <div className="absolute inset-0 z-10 backdrop-blur-[2px] rounded-2xl" style={{ background: "rgba(255,255,255,0.5)" }} />

        {/* Sidebar percakapan */}
        <div className="w-64 shrink-0 border-r border-gray-100 flex flex-col">
          <div className="p-3 border-b border-gray-50">
            <input type="text" placeholder="Cari percakapan..." className="w-full px-3 py-2 text-sm bg-gray-50 rounded-lg border border-gray-100 focus:outline-none" readOnly />
          </div>
        ...
        {active && activeConv ? (
          <div className="flex-1 flex flex-col">
            ...
            <div className="p-4 border-t border-gray-100 flex gap-3">
              <input
                type="text"
                placeholder="Fitur ini segera hadir..."
                className="flex-1 px-4 py-2.5 text-sm bg-gray-50 border border-gray-100 rounded-xl focus:outline-none cursor-not-allowed"
                readOnly
              />
              <button disabled className="px-4 py-2.5 rounded-xl text-white text-sm font-semibold opacity-40 cursor-not-allowed" style={{ background: "var(--primary)" }}>
                Kirim
              </button>
            </div>
          </div>
        ) : (
          <div className="flex-1 flex items-center justify-center text-gray-400 text-sm">Pilih percakapan</div>
        )}
      </div>
    </div>
  );
