            ...
            <div className="mt-4 space-y-4">
              {tenantGroups.map((group) => (
                <div key={group.tenantKey}>
                  <p className="text-xs font-semibold text-gray-600 mb-2 flex items-center gap-1"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 22V12h6v10"/></svg> {group.tenantName}</p>
                  <div className="space-y-1 ml-4">
                    {group.items.map((item) => {
                      const imgUrl = getAssetUrl(item.product?.images?.[0]?.image_path);
                      return (
                        <div key={item.id} className="flex items-center gap-2.5">
                          <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 bg-gray-100 border border-gray-100 flex items-center justify-center">
                            {imgUrl
