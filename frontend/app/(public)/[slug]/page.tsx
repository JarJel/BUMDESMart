...
import { tokoList, DOKUMEN_META } from "@/lib/data/dummy";
...
export default function TokoProfilePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <TokoContent toko={toko} />;
}

function TokoContent({ toko }: { toko: typeof tokoList[number] }) {
      ...
      <div
        ...
        style={{
          ...
          backgroundImage: toko.foto
            ? `linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.6)), url('${toko.foto}')`
            : `linear-gradient(135deg, var(--primary-dark), var(--primary))`,
          backgroundSize: "cover",
          backgroundPosition: "center",
        }}
      >
        {/* Banner size hint — tampil hanya jika tidak ada foto */}
        {!toko.foto && (
          <div className="absolute inset-0 flex items-center justify-center">
            <p className="text-white/50 text-sm font-medium text-center px-4">
              Upload banner toko<br />
              <span className="text-xs font-normal">Ukuran ideal: 1200 × 400px · rasio 3:1 · JPG/PNG · maks. 2MB</span>
            </p>
          </div>
        )}
        ...
        {/* Info overlay */}
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent pt-12 pb-5 px-4">
          <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 flex items-end gap-4">
            <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl border-2 border-white/30 overflow-hidden shrink-0 flex items-center justify-center" style={{ background: "var(--primary)" }}>
              <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
              </svg>
            </div>
            <div className="flex-1 min-w-0">
              <h1 className="text-lg sm:text-xl font-bold text-white leading-tight truncate">{toko.nama}</h1>
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <div className="flex items-center gap-0.5">
                  {starsArr.map(s => <StarIcon key={s} className={`w-3 h-3 ${s <= Math.round(toko.rating) ? "text-yellow-400" : "text-white/30"}`} />)}
                  <span className="text-white/80 text-xs ml-1">{toko.rating}</span>
                </div>
                <span className="text-white/40 text-xs">·</span>
                <span className="text-white/70 text-xs">{toko.totalPenjualan.toLocaleString("id")} terjual</span>
                <span className="text-white/40 text-xs">·</span>
                <span className="text-white/70 text-xs">{toko.totalProduk} produk</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <aside className="w-full lg:w-64 shrink-0 space-y-4">
            ...
            <div className="bg-white rounded-2xl p-4 border border-gray-100 space-y-3">
              ...
              {[
                { icon: "M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z", label: "Pemilik", val: toko.pemilik },
                { icon: "M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0zM15 11a3 3 0 11-6 0 3 3 0 016 0z", label: "Lokasi", val: toko.lokasi },
                { icon: "M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z", label: "Kategori", val: toko.kategori },
              ].map((row) => (
            ...
            <div className="bg-white rounded-2xl p-4 border border-gray-100">
              ...
              <div className="space-y-2">
                {(toko.dokumen as Dokumen[] | undefined)?.map((doc) => {
            ...
            <div className="bg-white rounded-2xl p-4 border border-gray-100">
              ...
              <p className="text-xs text-gray-500 leading-relaxed">{toko.deskripsi}</p>
            </div>
          ...
          <main className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-bold text-gray-900">Produk dari {toko.nama}</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {toko.produk.map((produk) => (
                <ProductCard
                  key={produk.id}
                  product={{ ...produk, tokoSlug: toko.slug, tokNama: toko.nama, tokoPemilik: toko.pemilik }}
                  compact
                />
              ))}
            </div>
