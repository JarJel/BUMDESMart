...
function PhotoSlider({ photos }: { photos: string[] }) {
  ...
  return (
    <div className="relative w-full aspect-video bg-gray-100 rounded-xl overflow-hidden mb-3">
      ...
      {urls.length > 1 && (
        <>
          <button onClick={() => setIdx((i) => (i - 1 + urls.length) % urls.length)} className="absolute left-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/40 text-white flex items-center justify-center">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
          </button>
          <button onClick={() => setIdx((i) => (i + 1) % urls.length)} className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/40 text-white flex items-center justify-center">
...
function BeritaCard({ item, onOpenLeaderboard }: { item: Berita; onOpenLeaderboard: (item: Berita) => void }) {
    ...
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
      <div className="p-5">
        <div className="flex items-start gap-3 mb-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1.5">
              <span className="text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full bg-orange-50 text-orange-600">
        ...
        {item.content.length > 150 && (
          <button onClick={() => setOpen((v) => !v)} className="text-xs text-orange-500 mt-1 font-medium">
          ...
          <div className="mt-4 pt-4 border-t border-gray-50 space-y-3">
            {item.event_date && (
              <div className="flex items-center gap-2 text-sm text-gray-600">
                ...
                <span>Tanggal Event: <strong>{formatDate(item.event_date)}</strong></span>
              </div>
            )}
                  ...
                  {(item.registrations_open || registered) && (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-colors disabled:opacity-50 ${
                        registered ? "bg-red-50 text-red-600 hover:bg-red-100" : "bg-orange-500 text-white hover:bg-orange-600"
                      }`}
                    >
...
function LeaderboardModal({ berita, onClose }: { berita: Berita; onClose: () => void }) {
  const [list, setList] = useState<Peserta[]>([]);
  const [myRank, setMyRank] = useState<number | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get(`/my/berita/${berita.id}/peserta`).then((r) => {
      setList(r.data.data ?? []);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        {myRank && (
          <div className="mx-5 mt-4 p-3 bg-orange-50 rounded-xl text-sm text-orange-700 font-semibold text-center">
        ...
        <div className="p-5 space-y-2 max-h-96 overflow-y-auto">
          {loading ? (
            ...
            list.map((p, i) => (
              <div key={p.user_id} className={`flex items-center gap-3 p-3 rounded-xl ${p.is_me ? "bg-orange-50 border border-orange-100" : "bg-gray-50"}`}>
                <span className={`text-xs font-bold w-6 text-center shrink-0 ${i < 3 ? "text-amber-500" : "text-gray-400"}`}>{i + 1}</span>
                {p.photo ? (
                  <img src={getFileUrl(p.photo) ?? ""} alt={p.name} className="w-8 h-8 rounded-full object-cover shrink-0" />
                ) : (
                  <div className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-xs font-bold text-orange-600 shrink-0">{p.initials}</div>
                )}
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-medium truncate ${p.is_me ? "text-orange-700" : "text-gray-900"}`}>
                    {p.name}
                    {p.is_me && <span className="ml-1.5 text-[10px] bg-orange-500 text-white px-1.5 py-0.5 rounded-full">Kamu</span>}
                  </p>
...
export default function PengirimBeritaPage() {
  const [items, setItems] = useState<Berita[]>([]);
  const [loading, setLoading] = useState(true);
  const [leaderboardItem, setLeaderboardItem] = useState<Berita | null>(null);

  useEffect(() => {
    api.get("/my/berita").then((r) => setItems(r.data.data?.data ?? [])).finally(() => setLoading(false));
  }, []);
    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2].map((i) => <div key={i} className="bg-white rounded-2xl border border-gray-100 p-5 h-40 animate-pulse" />)}
        </div>
      ) : items.length === 0 ? (
      ...
      {leaderboardItem && <LeaderboardModal berita={leaderboardItem} onClose={() => setLeaderboardItem(null)} />}
    </div>
  );
}
