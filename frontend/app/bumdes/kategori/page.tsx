...
export default function BumdesKategoriPage() {
  ...
  const handleSeedDefaults = () => setConfirmSeed(true);

  const executeSeedDefaults = async () => {
    setSeeding(true);
    setConfirmSeed(false);
    try {
      const res = await api.post("/admin/categories/seed-defaults");
      toast.success(res.data.message);
      fetchCategories();
    } catch (err: any) {
      toast.error(err.response?.data?.message ?? "Gagal memuat kategori standar.");
    } finally {
      setSeeding(false);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6 max-w-3xl">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            disabled={seeding}
            className="inline-flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-semibold border border-gray-200 text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50"
          >
            <svg className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
            </svg>
            <span className="hidden sm:inline">{seeding ? "Memuat..." : "Muat Kategori Standar"}</span>
            <span className="sm:hidden">{seeding ? "Memuat..." : "Standar"}</span>
          </button>
          <button
            onClick={() => openAdd()}
            className="inline-flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-semibold text-white"
      ...
      <ConfirmDialog
        open={confirmSeed}
        variant="warning"
        title="Muat Kategori Standar?"
        description="Akan memuat 8 kategori utama beserta sub-kategorinya (total ~36 kategori). Kategori yang sudah ada tidak akan diduplikat."
